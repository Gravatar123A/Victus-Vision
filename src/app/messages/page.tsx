  ...
  const fetchConversations = async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (data.conversations.length > 0 && !selectedConvo) {
          ...
          <div className={`${showSidebar ? "block" : "hidden"} md:block w-full md:w-80 border-r border-[var(--border-color)] flex-col`} style={{ display: showSidebar ? "flex" : undefined }}>
            ...
            <div className="flex-1 overflow-y-auto">
              {loadingConvos ? (
                ...
                <div className="flex flex-col flex-1 items-center justify-center p-8 text-center text-[var(--muted)]">
                ...
                conversations.map(convo => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 p-4 hover:bg-[var(--surface-alt)] transition-colors text-left ${selectedConvo?.user.id === convo.user.id ? "bg-[var(--surface-alt)]" : ""}`}
                    >
